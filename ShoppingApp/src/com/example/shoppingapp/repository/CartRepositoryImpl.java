...
import com.example.shoppingapp.repository.DbConnectionImpl;
...
public class CartRepositoryImpl implements CartRepository{
	...
	public List<CartEntry> getCartList(User user) throws  DataException{
		...
		try {
			...
			String selectQuery = " SELECT product_code, quantity_in_cart"
							   + " FROM carts WHERE id = ?";
	...
	@Override
	public void addProductInCart(User user, Product product, 
			int quantityFromCart,  boolean duplicateFlag)  throws  DataException{
		...
		try {
			...
			if(duplicateFlag ==true) {
				...
				selectQuery = " SELECT quantity_in_cart FROM carts"
							+ " WHERE id = ?"
							+ " AND product_code = ?";
				ps =  conn.prepareStatement(selectQuery);
				...
				updateQuery = " UPDATE carts SET quantity_in_cart = ? WHERE id= ?"
							+ " AND product_code= ?";
				...
				updateQuery = " INSERT into carts (id, product_code, quantity_in_cart)"
				        	+ " values (?, ?, ?)";
	...
	public void updateProductInCart(User user, Product product, int quantity) throws  DataException{
		...
		try {
			...
			String query = " UPDATE carts SET quantity_in_cart = ?"
						 + " WHERE id= ?"
						 + " AND product_code= ?";
	...
	public void checkOut(User user) throws  DataException{
		...
		try {
			...
			String updateQuery = " UPDATE inventory"
							   ...
							   + " (inventory.quantity - carts.quantity_in_cart)"
							   + " WHERE inventory.product_code = carts.product_code"
							   + " AND carts.id= ?";
	...
	public boolean isProductExist(Product product, User user) throws  DataException {
		...
		try {
			DbConnection dbConnection = new DbConnectionImpl();
			conn = dbConnection.getConnection();
			String selectQuery = " SELECT product_code FROM carts"
			                   + " WHERE id = ?"
							   + " AND product_code = ?";
